...
namespace TS {
    ...
    void LimitOrder::buy(Trader& trader, Stock& stock, OrderBook& orders) {
        ...
        std::string history;
        history += "[limit] " + trader.getName() + " " + trader.getSurname() + " '" + std::to_string(m_traderId) + "' buying "
    ...
    void LimitOrder::sell(Trader& trader, Stock& stock, OrderBook& orders) {
        double sum = m_expected_price * m_count;
        std::string history;
        history += "[limit] " + trader.getName() + " " + trader.getSurname() + " '" + std::to_string(m_traderId) + "' selling "
        + m_stock_name + " '" + std::to_string(m_transactionId) + "' ";
        if (stock.getPrice() >= m_expected_price) {
